...
#     How to rotate toolface:
#         Express toolface at given position as a vector (which is continous wrt. to rotation)
#         Rotate everything in cartesian coordinates
...
#         # Dogleg toolface uses minimum curvature to handle final position increments,
#         # and min curve breaks down at arc angles larger than pi. Handle by splitting in parts
#         theta = dls * md_inc

#         # NOTE
#         # This entire approach is VOID, as it assumes that min_curve implicitly
#         # uses a constant toolface, which is not the case. And thus, addition
#         # breaks down, and we cannot handle theta > pi with this approach.

#         if theta >= np.pi:
#             raise ValueError
#             # step_ang = 3/4 * np.pi
#             # n_steps = np.ceil(theta / step_ang) + 1
#             # print("n_steps:", n_steps)
#             # n_steps = int(n_steps)
#             # n_steps = max(n_steps, 3)

#             # ang = np.linspace(0, theta, num=n_steps)

#             # md_incs = ang / dls
#             # md_incs = np.diff(md_incs)

#             # # Sanity checks
#             # assert( abs(ang[-1] - theta) < 1e-3 )
#             # assert ( abs( md_inc - sum(md_incs)) <1e-3 )
#             # assert( max (md_incs * dls) < np.pi)
...
#             # inc_part = inc_upper
#             # azi_part = azi_upper
#             # toolface_part = toolface

#             # for i in range(0, len(md_incs)):
#             #     dnorth_part, deast_part, dtvd_part, inc_part, azi_part, toolface_part \
#             #         = dogleg_toolface_inner(inc_part, azi_part, toolface_part, dls, md_incs[i])
#             #     # Accumulate position increments
#             #     dnorth = dnorth + dnorth_part
#             #     deast = deast + deast_part
#             #     dtvd = dtvd + dtvd_part
            
#             # inc_lower = inc_part
#             # azi_lower = azi_part
#             # tf_lower = toolface_part

#             # return np.array((dnorth, deast, dtvd, inc_lower, azi_lower, tf_lower))
#         else:
#             # Theta is less than pi, go a head with one call
#             return dogleg_toolface_inner(inc_upper, azi_upper, toolface, dls, md_inc)


# # def dogleg_toolface_inner(inc_upper, azi_upper, toolface, dls, md_inc):
# #     """
...
# #     # Inclination at lower station
# #     # TODO Look hard for a bug here!
# #     inc_lower = np.arccos(partial_1 - partial_2)
    
# #     partical_3 = sin_tf * np.sin(theta)  

# #     # Calculate increment to azimuth
# #     delta_azi = 0.0
...
# #     # In practice, the result are curves in a constant azimuthal plane
# #     # that is only build, no turn.
# #     if inc_upper == 0 or inc_upper == np.pi:
# #         azi_lower = toolface
    

# #     # While the dogleg toolface produces a
